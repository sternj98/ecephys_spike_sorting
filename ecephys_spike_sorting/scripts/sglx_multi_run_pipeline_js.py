...
def run_ecephys(npx_directory,results_directory,
                                                modules = [
                                                'kilosort_helper',
    ...
    refPerMS_dict = {'default': 2.0, 'cortex': 2.0, 'medulla': 1.5, 'thalamus': 1.0} # refractory period in ms for quality metrics

    # now a kwarg
    # threhold values appropriate for KS2, KS2.5
    # ksTh_dict = {'default':'[10,4]', 'cortex':'[10,4]', 'medulla':'[10,4]', 'thalamus':'[10,2.5]'}
    ...
    # npx_directory = r'Z:\\HarveyLab\\Tier1\\Cindy\\EphysData\\CY\\23\\211215\\000000'
    npx_directory_full = os.path.join(npx_directory,os.listdir(npx_directory)[0])
                    #   Z:\HarveyLab\Tier1\Cindy\EphysData\CY\23\211215\000000
    ...
    # run_specs = [									
    # 						['CY23_LLP_20211215', '0', 'start,end', '0', ['thalamus'] ]
    # ]
    ...
    npx_directory_split = npx_directory.split(os.sep)
    ...
    catGT_cmd_string = '-prb_fld -out_prb_fld -inarow=1 -apfilter=butter,12,300,10000 -lffilter=butter,12,1,500 -gfix=0.4,0.10,0.02 '

    # TTL information we're pulling out for sync!
    ni_present = True
    ni_extract_string = '-SY=0,-1,6,500 -XA=3,2,0,0 -XD=-1,0,500 -XD=-1,1,0 -iXD=-1,1,0'
    ...
    niStream_sync_params = 'XD=-1,0,500' # 'XA=0,1,3,500'   # copy from ni_extract_string, set to None if no Aux data, no spaces
    ...
    if not os.path.exists(catGT_dest): # added 11/11/22 JS
        os.makedirs(catGT_dest)
    ...
    for spec in run_specs:
        ...
        # prb0_fld = os.path.join(npx_directory, prb0_fld_name) # changed 12/30/21 JS
        ...
        for i, prb in enumerate(prb_list):
            ...
            # input_data_directory = os.path.join(npx_directory, prb_folder) # difference in file naming? JS 12/29
        ...
        if runTPrime:
            ...
            im_ex_list = ''
            for i, prb in enumerate(prb_list):
                sync_extract = '-SY=' + prb + ',-1,6,0' # ',-1,6,500'
